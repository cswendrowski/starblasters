...
func set_pixels(amount):
	...
	# Center Flares/Blobs on the Star core for any relative_scale (Roman,
	# 2026-05-18: bumping relative_scale to 3.0 broke the old `-amount*0.5`
	# offset which only worked at relative_scale=2). Star core spans
	# (0,0)→(amount,amount); the surrounding square of size
	# amount*relative_scale needs to be offset by half that span minus
	# half the core span so its center sits on the core's center.
	var flare_offset: float = (amount - amount * relative_scale) * 0.5
	$StarFlares.position = Vector2(flare_offset, flare_offset)
	$Blobs.position = Vector2(flare_offset, flare_offset)
