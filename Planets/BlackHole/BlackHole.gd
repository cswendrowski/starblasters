...
func set_rotates(r):
	# galaxy_backdrop calls this with a bool (`rng.randf() < 0.7`). Coerce
	# to float so the third-party asset's `r + 0.7` doesn't error on
	# bool+float (Roman, 2026-05-17: long-standing smoke harness noise).
	var rf: float = (1.0 if r else 0.0) if typeof(r) == TYPE_BOOL else float(r)
	$Disk.material.set_shader_parameter("rotation", rf + 0.7)
