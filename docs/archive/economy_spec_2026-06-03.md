**⚠️ ARCHIVED 2026-06-15 — historical snapshot, not current design.** Superseded by: live economy in scripts/autoload/run_state.gd (bounty + Materials; outpost is a hub).
Kept for design history; do not cite as the live spec.
> NOTE: the proposed two-currency model (bounty vs station-supply) and ace-kill-chain were NEVER built — live economy uses bounty + Materials (power currency, not survival).
