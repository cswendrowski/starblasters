...
When the enemy drifts off the bottom (or fully off a side, if its pattern sets `allow_side_exit`), instead of freeing, it plays a **despawn** animation: it tweens back UP through the parallax layers as a shrunk, depth-tinted ghost (speed-proportional to its on-field `move_speed`). A recycling enemy reports `is_recycling() = true`, which the off-screen hit-immunity guard ([`enemy_base.gd`](../../scripts/enemies/enemy_base.gd)) and the smart bomb use to skip it mid-fly-back. Note: a recycler still counts as a live combatant for the level-clear gate ([`director._live_combatants_present`](../../scripts/levels/director.gd)).

The **new contract (despawn + credit + conducted re-entry, 2026-07-06):** at the END of the fly-back the enemy does NOT restore itself at the top. Instead [`RecycleController.recycle`](../../scripts/effects/recycle_controller.gd) looks up the wave director (scene-tree group `"wave_director"`), hands it a **recycle credit** — the enemy's source `WaveSpec` plus its remaining `recycle_passes` (stashed at spawn as the `recycle_source_spec` meta) — and `queue_free`s the node. The director queues the credit in `_recycle_pool`; at each **wave boundary** (and once more at level end) it drains the pool as its own **conducted sweep rows** (`_drain_recycle_pool`, reusing the sweep-row idiom), spawning ONE fresh enemy per credit and stamping the carried remaining passes back onto it (so the pass budget doesn't reset — chaff can't loop forever). A returning unit re-enters at full HP (fresh arrival). Because pending credits are outstanding combatants, `_live_combatants_present` treats a non-empty pool as "not clear", and the level-end drain waits out any in-flight fly-back — so the kill-gate math holds and the level can't end early or hang. This replaces the old behavior where a missed enemy restored itself at the top and resumed its pattern un-conducted (the source of stragglers weaving through formations).

**Legacy fallback:** in dev labs, the Enemy Bench, and test harnesses there is NO director in the `"wave_director"` group. `RecycleController.recycle` detects this and keeps the OLD immediate restore+resume path (the enemy re-enters at the top with its pattern), so every dev tool behaves exactly as before.
...
**Parallax recycling:**
Pattern-driven enemies default to `offscreen_mode = CYCLE_BOTTOM`. When they drift off the bottom, they tween back up through the parallax as a despawn animation. In a **real level** the ghost is then freed and the director re-spawns a fresh replacement as a conducted re-entry row at the next wave boundary (full HP; see "Off-screen recycling" above for the full despawn+credit contract). In a **dev scene with no director** the legacy path runs instead: the SAME instance cycles and keeps whatever health it had (health is set once at spawn; the fly-back does not refresh it). Call `is_recycling()` to check if an enemy is mid-fly-back. The fly-back timing/look is owned by [`RecycleController`](../../scripts/effects/recycle_controller.gd) (tunable live via the Recycle Tuner dev scene).
