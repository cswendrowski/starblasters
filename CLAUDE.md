...
- **Lab preview contract (2026-07-17):** a dev tool that renders gameplay hosts it in `HdScreen.make_play_subviewport(...)` — or, if the viewport must be hand-built/scene-authored, calls `HdScreen.apply_native_parity(vp)` on it (SubViewports do NOT inherit the project's `hdr_2d` / `snap_2d_transforms_to_pixel`; missing parity = shimmering pixel art + wrong additive blends). Register the in-viewport world node in the `"bullet_world"` group (BulletWorld-routed spawns land inside instead of the 1920 window's top-left) and add `HdScreen.verify_native_subviewport.call_deferred(vp, "<lab>")` so misconfiguration errors loudly. Exit cleanup is structural: `SceneTransition.change_scene` sweeps stray root-parented `enemies`/`enemy_bullets`/`bullets` group members at every scene hop — don't hand-roll per-lab sweeps.
...
- `scripts/enemies/enemy_core.gd` (alongside `enemy_base.gd` since the 2026-06-14 reorg) — extends the base, adds `movement: Resource` + `shoot_pattern: Resource` slots. Ticks components (beams attach via `MountBuilder`, or from a bespoke host — enemy_core has no beam code of its own). **Firing is unified through the mount engine (2026-07-07): a hull `shoot_pattern` is realized as a `MountComponent` (`hull_pattern`) by `_realize_shoot_pattern_mount` in `_ready`, so cadence/gate/path-phase/on-phase all run on `MountComponent` + `FiringScheduler` — there is no separate hull ShootTimer firing loop anymore. The retired hull `ShootTimer` node + its inert `_on_shoot_timer_timeout` stub were fully removed from `enemy_core.gd` and all non-boss enemy `.tscn` files (2026-07-07). Bosses (`boss_base.gd` + scenes under `scenes/enemies/bosses/`) keep their OWN live `ShootTimer` + `_on_shoot_timer_timeout` — that path is unrelated.**
- `patterns/` — movement Resources; subclass `movement_pattern.gd`, override `compute_step(enemy, delta) -> Vector2`.
- Custom enemies (bomber) extend `enemy_base.gd` directly, expose `hull`/`max_hull` + `hull_changed` signal. (Bulwark was de-bespoked 2026-07-09 — now plain `enemy_core` + Drift movement + ShieldComponent in its `.tscn`.)
...
- **Component** — `scripts/enemies/components/` (`EnemyComponent` base + `ShieldComponent`, `OrbitComponent`, `BeamChargeVisual`) plus `MountComponent` under `scripts/enemies/mounts/` (payload/gun/launcher realization — folds the retired `EmitterComponent`). Optional hooks (on_start/process/hit/death/leave); registry on `enemy_base`, ticked by `enemy_core`, duplicated per-instance. Damage routes through `_components_hit` before hull.
...
- **Motion-clarity speed scale**: enemy/projectile speeds follow the **1–8 px/frame rung scale** in `scripts/systems/clarity.gd` (rung = multiple of 60 px/s; **8 px/f = 480 px/s is the hard ceiling** — past it objects strobe/ghost at 480×270). Author speeds on a rung (`Clarity.snap_to_rung` / `proposed_speed`); lasers sit at 8, chaff bullets 1–3, fast enemies ~4. Keep authored speeds at/under the 8 px/f ceiling — enemy speeds are used as authored (the old +5%/sector scaling + auto-clamp was retired 2026-06-23 with the single-sector switch). `max_fps=60` (project.godot) is intentional — pixel-art motion reads cleanest at the 60 Hz tick with snap on; do not raise it. Re-audit with `tools/clarity_audit.gd`.
