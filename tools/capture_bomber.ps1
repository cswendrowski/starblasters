param([int]$Fps = 24, [string]$OutName = "bomber")
...
$FrameDir = Join-Path $OutDir 'bomber'
& $GODOT --path $Repo -s 'res://tools/capture_bomber.gd' | Out-Null
...
$FilterGraph = 'scale=640:-1:flags=neighbor,split[s0][s1];[s0]palettegen=stats_mode=diff[p];[s1][p]paletteuse=dither=bayer:bayer_scale=5'
