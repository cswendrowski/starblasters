param([int]$Fps = 24, [string]$OutName = "shadow_test")
...
$FrameDir = Join-Path $OutDir 'shadow_test'
& $GODOT --path $Repo -s 'res://tools/capture_shadow_test.gd' | Out-Null
...
$FilterGraph = 'scale=540:-1:flags=neighbor,split[s0][s1];[s0]palettegen=stats_mode=diff[p];[s1][p]paletteuse=dither=bayer:bayer_scale=5'
