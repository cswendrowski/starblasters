param(
  [int]$Fps = 24,
  [string]$OutName = "horizontal_proof"
)
...
$FrameDir = Join-Path $OutDir 'horizontal_proof'
if (-not (Test-Path $OutDir)) { New-Item -ItemType Directory $OutDir | Out-Null }

& $GODOT --path $Repo -s 'res://tools/capture_horizontal_proof.gd' | Out-Null
...
$FilterGraph = 'scale=960:-1:flags=lanczos,split[s0][s1];[s0]palettegen=stats_mode=diff[p];[s1][p]paletteuse=dither=bayer:bayer_scale=5'
