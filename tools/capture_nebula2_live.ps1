...
$FrameDir = Join-Path $OutDir 'nebula2_live'
if (-not (Test-Path $OutDir)) { New-Item -ItemType Directory $OutDir | Out-Null }

& $GODOT --path $Repo -s 'res://tools/capture_nebula2_live.gd' | Out-Null
...
$FilterGraph = 'scale=400:-1:flags=lanczos,split[s0][s1];[s0]palettegen=stats_mode=diff[p];[s1][p]paletteuse=dither=bayer:bayer_scale=5'
