param([int]$Fps = 30, [string]$OutName = "projectile_glow")
$ErrorActionPreference = 'Continue'
# Godot 4.6.3 standalone (single binary editor + export). Must run WITHOUT
# --headless: the shader glow needs a real GPU renderer (headless's dummy
# renderer returns null viewport textures and won't compile the shader).
...
$FrameDir = Join-Path $OutDir 'projectile_glow'
if (-not (Test-Path $OutDir)) { New-Item -ItemType Directory $OutDir | Out-Null }
& $GODOT --path $Repo --script 'res://tools/capture_projectile_glow.gd' | Out-Null
...
$FilterGraph = 'scale=960:-1:flags=neighbor,split[s0][s1];[s0]palettegen=stats_mode=diff[p];[s1][p]paletteuse=dither=bayer:bayer_scale=5'
