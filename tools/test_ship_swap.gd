...
# Run.ship_variant = 1 (the Falchion — the old "ship B" slot) + a chosen livery, then instantiates
# main.tscn as a CHILD (mirrors SceneTransition.change_scene entering combat — NOT the busy initial
# scene), and confirms main.gd swapped the baked Starblaster for the Falchion with its double engine
# markers + livery honored. Run:
...
func _process(_dt: float) -> void:
	...
	# The Falchion has EngineL + EngineR (the Starblaster has a single Engine). Confirm it installed.
	...
	if not (has_l and has_r):
		lines.append("FAIL expected Falchion double engine markers"); fails += 1
	# Body texture should be the Falchion body sheet.
	...
	if not tex_path.contains("player_falchion"):
		lines.append("FAIL expected player_falchion body texture"); fails += 1
