...
# BeamEmitter draws LOCAL_FORWARD, so the hull's rotation aims it for free. Two
# variants via `aim_at_player`: false = face straight down; true = rotate to track.
#
...
# Rotate the hull so its front (FWD_LOCAL) points at the aim target — straight down
# for the aim-down variant, or smoothly toward the player for the tracker. The beam
# (a child of the hull) is carried along, so it always exits the front.
