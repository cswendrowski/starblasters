...
# PRESERVED FOR THE PLANNED BOMBER-WING-EVENT OVERHAUL (Roman 2026-06-01).
# This is the original V-formation "bomber wing" unit, moved aside when
# enemy_bomber.gd / enemy_bomber.tscn were repurposed into the new roster
# bomber (slow-descend tailgunner). It is NOT wired to any scene or roster
# entry right now — kept on disk so the wing event can be rebuilt from it
# rather than from git archaeology. Authored against the old 48×48 ship_6 art
# with hardcoded turret/tell coords, so the overhaul will likely re-fit the
# markers regardless.
#
