...
# Player FX Lab (Roman 2026-06-11). Runs the live player ship through hull damage
# levels so the damage tells — engine fire (engine_torch), damage smoke
# (damage_smoke_trail), engine torch, and the damage overlay — can be SEEN applying at
# each level, and verifies they react to max-HP changes. Also triggers the asteroid
# explosion + dust/smoke trail. The review vehicle for the damage-tell shader work.
# Plain Node2D scene (no SubViewport) so the world-space fx render here directly.
...
var _ship_dd: OptionButton = null
...
func _apply_hull() -> void:
	...
	# Belt-and-braces: re-emit so the FX re-evaluate the max-HP fraction even if hull
	# didn't change (e.g. only the max slider moved).
	if _player.has_signal("hull_changed"):
		_player.hull_changed.emit(mh, h)
	if _readout:
...
func _drift_asteroid() -> void:
	...
	# Asteroid._process drives its own descent + dust trail; just let it fall.
...
func _build_ui() -> void:
	...
	rail.position = Vector2(8, 8)
	rail.add_theme_constant_override("separation", 6)
	layer.add_child(rail)

	var title := Label.new()
	title.text = "PLAYER FX LAB"
	rail.add_child(title)

	_ship_dd = OptionButton.new()
	for s in ["Ship A", "Ship B", "Ship C"]:
		_ship_dd.add_item(s)
	_ship_dd.item_selected.connect(func(i): _spawn_player(i))
	rail.add_child(_ship_dd)

	rail.add_child(_mk_label("Hull (drag to damage)"))
	_hull_slider = _mk_slider(0, 3, 1, 3)
	_hull_slider.value_changed.connect(func(_v): _apply_hull())
	rail.add_child(_hull_slider)

	rail.add_child(_mk_label("Max Hull (max-HP test)"))
